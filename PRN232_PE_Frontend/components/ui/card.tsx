...
function Card({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      data-slot="card"
      className={cn(
        "bg-card/80 backdrop-blur-sm text-card-foreground flex flex-col gap-6 rounded-2xl border border-border/60 py-8 px-6 shadow-sm hover:shadow-md transition-shadow duration-200",
        className
      )}
      {...props}
    />
